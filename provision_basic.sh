...
PASSWD=$(htpasswd -bnBC 10 "" "${openai_password}" | tr -d ':\n')
USER="${openai_user}"

/usr/bin/docker pull ghcr.io/open-webui/open-webui:main
/usr/bin/docker run -d -p 80:8080 -v /etc/open-webui.d:/root/.open_web_ui -v /etc/open-webui.d:/app/backend/data --name openwebui ghcr.io/open-webui/open-webui:main
...
INSERT INTO user (
  id, name, email, role, profile_image_url, api_key,
...
Environment="OPENAI_API_BASE_URLS=${openai_base}"
